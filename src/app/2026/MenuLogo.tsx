...
export const MenuLogo: FunctionComponent<Props> = ({ data, style = {} }) => {
  ...
  const calcTranslateX = (index: number, total: number) => {
    ...
    return (
      Math.cos(Math.PI + (Math.PI / 2) * (position / (totalInLayer - 1))) *
      (index < 3 ? 60 : 100)
    );
  };
  ...
  const calcTranslateY = (index: number, total: number) => {
    ...
    return (
      Math.sin((Math.PI / 2) * (position / (totalInLayer - 1))) *
      (index < 3 ? 60 : 100)
    );
  };

  return (
    <div style={style} className="flex w-50 h-50">
      ...
      {data.map((element, index) => {
        return (
          <div
            ...
            style={{
              transform: menuOpen
                ? `translate(${calcTranslateX(index, data.length)}px, ${calcTranslateY(index, data.length)}px) rotate(720deg)`
